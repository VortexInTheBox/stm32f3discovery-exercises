/*
Simple Blink program with the stm32f3discovery board blue LED.
...
int main()
{
	...
	TIM6->CNT &= ~(0x0000FFFF); // Sets the counter to 0

	short int flag = 0;

	while (1)
	{
		if ((TIM6->SR & TIM_SR_UIF) == TIM_SR_UIF && !flag) //If the led is off
		{
			TIM6->SR &= ~TIM_SR_UIF;  // Resets the flag raised by the counter
			GPIOE->ODR |= GPIO_ODR_8; // Turns on the led
			flag = 1;
		}
		if ((TIM6->SR & TIM_SR_UIF) == TIM_SR_UIF && flag) //If the led is on
		{
			TIM6->SR &= ~TIM_SR_UIF;
